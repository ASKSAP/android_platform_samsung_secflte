...
static job_requeue_t wait_for_teardown_callback(void *data)
{
	...
	DBG1(DBG_CFG, "Teardown signal received. strongSwan going down");
...
static bool wait_for_teardown_cancel(void *data)
{
  ...
  DBG1(DBG_CFG, "wait_for_teardown_cancel");
  return FALSE;
}
...
{
	DBG1(DBG_CFG, "status of Android plugin changed: %s", 
	...
	if (success)
	{
		...
		switch(process_id)
	  	...
	  	case 0: /* child */
			...
			err |= setenv("INTERNAL_CIDR4", "32", TRUE);
			err |= setenv("ROUTES", "", TRUE); /* Routes already set */
       ...
       private_android_service_t *this, ike_sa_t *ike_sa, alert_t alert, va_list args)
{
  ...
  DBG1(DBG_CFG, "received alert %s", alert_map[alert]);
  ...
  switch(alert)
	...
	return FALSE; /*we are interested to get any more alarms.*/
      }
      break;
    default:
      /* do nothing */
       ...
       private_android_service_t *this,  ike_sa_t *ike_sa, ike_sa_state_t state)
{
  ...
  DBG1(DBG_CFG, "IKE SA state changed to %s", ike_state_map[state]);
...
int do_initiate(private_android_service_t *this)
{
	...
	DBG1(DBG_CFG, "Executing Android specific worker thread");
	...
	if (configs->auth_method == AUTH_XAUTH_INIT_PSK ||
	    configs->auth_method == AUTH_XAUTH_INIT_RSA )
	  {
	    this->creds->set_username_password(this->creds, 
					       ...
					       TRUE);
	    DBG1(DBG_CFG, "XAUTH Username & password stored.");
	  }
	...
	if (configs->auth_method == AUTH_XAUTH_INIT_RSA ||
	    configs->auth_method == AUTH_RSA)
	  {
		...
		!this->creds->add_private_key(this->creds, configs->rsa.private_key))
	      {
	...
	switch(configs->auth_method)
	  ...
	  case AUTH_XAUTH_INIT_PSK:
	    {	      
	      ikev1_psk_configs_create(configs,
				       &peer_cfg,
				       &child_cfg);
	  ...
	  case AUTH_XAUTH_INIT_RSA:
	    {
	      ikev1_rsa_configs_create(configs,
				       &peer_cfg,
				       &child_cfg);
	    }
	    break;
	  case AUTH_PSK:
	    {	      
	      ikev2_psk_configs_create(configs,
				       &peer_cfg,
				       &child_cfg);
	    }
	    break;
	  case AUTH_RSA:
	    {
	      ikev2_rsa_configs_create(configs,
				       &peer_cfg,
				       &child_cfg);
	    }
	    break;
